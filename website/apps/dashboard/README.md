...
This fork includes a local template mode so the dashboard works before Clerk or
backend storage is configured. Copy `env.example.txt` to `.env.local`, keep
`NEXT_PUBLIC_AUTH_BYPASS=true`, and run:
...
Then open `http://localhost:3015/dashboard/overview`.

See `docs/local-template-mode.md` for the full Clerk bypass path and how to turn
real Clerk auth back on.
