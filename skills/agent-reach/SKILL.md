...
(installs the upstream `agent-reach` CLI from PyPI + its free backends via
`agent-reach install --env=auto`, then runs `agent-reach doctor`). After that the
zero-config platforms (Exa web search, GitHub, web-read, YouTube, V2EX, RSS) work
immediately; login platforms (Twitter, Reddit, XiaoHongShu) unlock with
`agent-reach configure`. This is the upstream tool at full fidelity — PMM OS only
adds this bootstrap line.
<!-- END PMM-OS-SETUP -->
